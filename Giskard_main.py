...
if __name__ == "__main__": 
    # cd C:\Users\nacha\OneDrive\Documents\Data\developer-test-main\examples\example1
    Empire_json = pd.read_json('/exemple1/empire.json', typ='series')
    empire = Empire(Empire_json.countdown, Empire_json.bounty_hunters)

    MF_json = pd.read_json('millennium-falcon.json', typ='series') # problème s'il y exemple1/millennium-falcon.json
    ...
    routes = Millennium_falcon.read_ROUTES(millennium_falcon)
    G = Millennium_falcon.create_Graph(millennium_falcon)
    feasible_paths = Millennium_falcon.find_feasible_paths(millennium_falcon)
    acceptable_paths = Millennium_falcon.find_acceptable_paths(millennium_falcon)
    alternative_paths = Millennium_falcon.find_alternative_paths(millennium_falcon)
    
    odds = Millennium_falcon.give_me_the_odds(millennium_falcon)[0]
    optim_path = Millennium_falcon.give_me_the_odds(millennium_falcon)[1]
